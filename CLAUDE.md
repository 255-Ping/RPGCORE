...
| `suiteVersion` increments in `gradle.properties` | `docs/changelog/suite-<N>.md` summary + `docs/changelog.md` index row (see Rule 6) |
...
**Every `config.yml` resource must start with this header on line 1:**
```yaml
# Generated by: ${pluginName} v${version}
```
`processResources` in `buildSrc/src/main/kotlin/rpg.module.gradle.kts` already filters `config.yml`
and substitutes both tokens at build time — no extra Gradle work needed. Just ensure the line is
present in `src/main/resources/config.yml` when you create a new plugin or add a config file to an
existing one. Servers that already have the file on disk won't see it updated (Bukkit only copies
on first run), but fresh installs and test-server wipes will.
...
**No per-session changelog updates.** The granular dev-log format is retired. Instead, the
changelog is written **once per suite at transition time:**

When `suiteVersion` increments (e.g., 19 → 20):
1. Run `git log --oneline` since the previous suite bump to gather what changed.
2. Write a concise summary at `docs/changelog/suite-<old-N>.md` — 5–10 bullet highlights
   (new systems, major bug fixes, new plugins). Skip patch tweaks and minor cleanup.
3. Add a row to the `docs/changelog.md` index table with the highlights one-liner.

The archived per-session dev logs live at `docs/changelog/suite-N.md` with an "Archived" admonition.
Do **not** add entries to them during normal development.
...
| rpg-regions | `regionsVersion` | 0.6.0 |
| rpg-farming | `farmingVersion` | 0.1.0 |
| rpg-guilds | `guildsVersion` | 0.1.0 |
| rpg-enchanting | `enchantingVersion` | 0.6.0 |
...
| rpg-bossbar | `bossbarVersion` | 0.1.0 |
...
RpgServices.bossBar()       // BossBarService (rpg-bossbar 0.1.0) — .track/.untrack/.clearAll; set by rpg-bossbar on enable
```
...
| `PlayerAttackAbilityListener` / `PlayerBlockAbilityListener` / `PlayerKillAbilityListener` | Event listeners for `on_attack`, `on_block`, `on_kill` triggers |
| `PlayerPassiveAbilityTask` | Repeating task that fires `passive` trigger bindings; interval: `abilities.passive-interval-ticks` |
| `OwnedMobTracker` | Tracks mobs spawned via `spawn_mob{owned=true}`; enforces per-caster cap (`abilities.spawn-mob.max-per-caster`); despawns all owned mobs when caster logs out |
...
| `MainMenuGui` / `MainMenuListener` / `MenuCommand` | Hub GUI (54-slot); persistent COMPASS item in hotbar slot 8 tagged via PDC; right-click or `/menu` opens it; nested GUIs get a Back callback |
| `SkillsGui` / `WalletGui` | Per-player skill overview GUI and economy summary GUI; both follow the back-callback nav standard |
| `AchievementGui` | Achievement browser GUI; unlocked = LIME_DYE, locked = GRAY_DYE |

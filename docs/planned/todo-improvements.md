...
### ✅ Repurposed Vanilla Bars: Remove XP Bar (`rpg-core`) — shipped

The `vanilla-xp-bar` config key was only a stub — the feature (writing skill XP to the vanilla bar) was never implemented in Java. Removed the config key and its comment block from `config.yml` and updated `docs/core/vanilla-suppression.md` to note the XP bar is left untouched so vanilla enchanting displays correctly. No `/skill pin` command or `rpg.core.skill.pin` permission existed to remove.

---

### ✅ Add More Example Status Effects (`rpg-core`) — shipped

Added six new effects to `status-effects/example.yml`: `weakness` (−30% damage, −20 strength flat), `vulnerability` (−35% defense, −10 true_defense), `mana_drain` (−40% max_mana, −80% mana_regen), `blindness` (−60% crit_chance, −40% magic_find), `speed_boost` (+40% speed), and `thorns` (+20 defense, +5 true_defense — with a code comment that reflect mechanic needs a StatusEffectService hook). The existing `slow` and `regen` entries cover the remaining requested categories already.

---

### ✅ Fill Out Example Crafting Recipes (`rpg-crafting`) — shipped

Replaced the commented-out stubs in `recipes/example.yml` with four live recipes: `shaped_iron_shortsword` (vanilla IRON_INGOT + STICK → custom `iron_shortsword`), `shaped_crude_iron_ingot` (9× IRON_NUGGET → `crude_iron_ingot`), `shapeless_antidote_flask` (`red_gem` + GLASS_BOTTLE → `antidote_flask`), and `shaped_red_gems` (REDSTONE diamond pattern → 3× `red_gem`). All four recipes use items already in `items/example.yml` and cover shaped-vanilla, shaped-RPG, shapeless, and multi-amount output patterns.

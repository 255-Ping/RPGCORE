...
64. 🔵 🔴 **Vault / Storage System** — per-player persistent storage vaults; admin-configures count (default 5), rows per vault (default 6), and unlock requirements (coins, MC XP, item, permission); `VaultSelectorGui` → `VaultGui`; `/vault [n]` command. See [New Features](todo-features.md).

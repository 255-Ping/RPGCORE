...
4. **Choose a backend** — YAML is fine for small servers. For anything bigger, set `persistence.backend: mysql` in `plugins/rpg-core/config.yml` and fill in credentials. First start with MySQL creates the schema automatically.
5. **Set vanilla suppression** — defaults cancel everything. If you want any vanilla mechanic back, set its flag to `false` in `plugins/rpg-core/config.yml` under `vanilla-suppression`.
...
1. **Define the block** in `plugins/rpg-core/blocks/my_ores.yml`:
...
2. **Define the item** (if you want a custom pickaxe that can mine it) in `plugins/rpg-core/items/tools.yml`:
...
1. Define in `plugins/rpg-core/mobs/forest_mobs.yml`:
...
3. **Place a spawner**: get your wand (`/rpg wand`), stand at the spawn point, run `/spawner create forest_goblin`.

---

## Authoring a custom ability

1. Define in `plugins/rpg-core/abilities/spells.yml`:
...
2. Attach to a wand item in `plugins/rpg-core/items/wands.yml`:
...
One wand, multiple modes. Get it: `/rpg wand`

| Mode | How to switch | Use |
|---|---|---|
| `region` | `/rpg wand region` | L-click corner 1, R-click corner 2 → `/region create <id>` |
| `dungeon` | `/rpg wand dungeon` | Same as region but for dungeon volumes |
| `spawner` | `/rpg wand spawner` | R-click a location → places a spawner |
| `hologram` | `/rpg wand hologram` | R-click a location → anchor for `/hologram create <id>` |
...
|---|---|---|
| Custom block breaks instantly | Block isn't PDC-tagged at that location | Use `/rpg block give <id>` to place it, or `/rpg block convert` |
| Nametag not showing | `rpg-hud` not loaded, or player joined before hud enabled | Ensure rpg-hud jar is in plugins; ask player to relog |
| Abilities not firing on right-click | Item has `Type: MATERIAL` instead of `SWORD`/`WAND` | Fix `Type:` in item YAML, reload |
| MySQL connection fails on startup | Wrong credentials, DB doesn't exist, or firewall | Check `config.yml` credentials; falls back to YAML with a warning |
| Stat shows 0 in `/stats` | Addon that sources the stat (e.g., rpg-combat for `strength`) not loaded | Install the relevant addon jar |
| Quest not progressing | `auto-complete: false` or wrong objective `Target` | Check target matches exact mob/block/item id; check `progress-action-bar: true` for feedback |
